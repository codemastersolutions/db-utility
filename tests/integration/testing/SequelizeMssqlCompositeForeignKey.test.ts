...
describe('Sequelize MSSQL Composite Foreign Key Integration', () => {
  maybeIt(
    'should create and enforce a composite foreign key with matching referenced columns',
    async () => {
      ...
      const migrationsDir = mkdtempSync(join(tmpdir(), 'dbutility-mssql-composite-fk-'));
      ...
      try {
        ...
        const fkRows = await connector.query<{
          ...
          constraint_column_id: number | string;
        }>(
          `
            SELECT
