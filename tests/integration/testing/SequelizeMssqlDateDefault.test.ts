...
describe('Sequelize MSSQL Date Default Integration', () => {
  maybeIt(
    ...
    async () => {
      ...
      const migrationsDir = mkdtempSync(join(tmpdir(), 'dbutility-mssql-date-default-'));
      ...
      try {
        ...
        const rows = await connector.query<{ definition: string | null }>(
          `
            ...
            WHERE t.name = 'DateDefaultSample'
